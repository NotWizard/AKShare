"""Derived-table slices — replaces db.load('derived_*')."""
...
router = APIRouter(prefix="/derived", tags=["data"])


@router.get("/monthly", response_model=DerivedFrame)
def derived_monthly(
...
@router.get("/quarterly", response_model=DerivedFrame)
